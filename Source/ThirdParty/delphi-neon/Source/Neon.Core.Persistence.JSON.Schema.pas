...
type
  ...
  protected
    ...
    /// <summary>
    ///   Flattens a [NeonUnwrapped] member's schema into the parent, mirroring
    ///   TNeonSerializerJSON.WriteMembers, which flattens whatever object the
    ///   member's writer produced. A fixed set of properties is copied into
    ///   AProperties and its required member names into ARequired. Shapes that
    ///   cannot be copied property-by-property contribute to the parent schema
    ///   instead: a bare $ref (a type hoisted into the definitions) adds an
    ///   "allOf" branch to AAllOf, and a map's open-ended "additionalProperties"
    ///   value schema goes to AAdditionalProperties. An unconstrained object
    ///   (an interface member) flattens to nothing, which is still right
    ///   because the parent leaves additional properties open. Returns False
    ///   only when the member does not serialize to an object at all (a string,
    ///   an array, ...), in which case the caller must nest it as usual
    /// </summary>
    function UnwrapMember(AMemberSchema, AProperties: TJSONObject; AAllOf: TJSONArray;
      var AAdditionalProperties: TJSONValue; var ARequired: TJSONArray): Boolean;

    /// <summary>
    ///   Writer for members of objects and records
    /// </summary>
    function WriteMembers(AType: TRttiType; AResult: TJSONObject; AAllOf: TJSONArray;
      var AAdditionalProperties: TJSONValue): TJSONArray;
...
function TNeonSchemaGenerator.UnwrapMember(AMemberSchema, AProperties: TJSONObject;
  AAllOf: TJSONArray; var AAdditionalProperties: TJSONValue; var ARequired: TJSONArray): Boolean;
var
  LProperties, LRequired, LAdditional, LType, LRef: TJSONValue;
  LCombined: TJSONObject;
  LCombinedAllOf: TJSONArray;
  LPair: TJSONPair;
  LIndex: Integer;
begin
  Result := False;

  // Mirror TNeonSerializerJSON.WriteMembers, which flattens exactly what the
  // member's writer produced as a TJSONObject and nests everything else, so a
  // member whose schema is not object-shaped (a string, an array, a number)
  // is left for the caller to nest as usual

  // A bare $ref: a type hoisted into the definitions because it participates
  // in a cycle. It still serializes to the object of its own members, flattened
  // into the parent, so the parent must also satisfy the target schema; allOf
  // says exactly that without having to inline the definition
  LRef := AMemberSchema.GetValue('$ref');
  if Assigned(LRef) then
  begin
    // Only refs of the shape "#/$defs/<name>" are generated by this unit, and
    // they always point at an object/record definition (ReferenceTo/DefNameFor);
    // any other ref is not one of ours and the member is nested instead
    if (LRef is TJSONString) and LRef.Value.StartsWith('#/' + DefsKeyword + '/') then
      AAllOf.AddElement(TJSONObject.Create.AddPair('$ref', LRef.Clone as TJSONValue))
    else
      Exit(False);
    Result := True;
  end;

  // A fixed set of properties: copied into the parent, exactly as before
  ...
  // A map: type "object" plus "additionalProperties". The serializer flattens
  // the map's key/value pairs into the parent, so the parent's own
  // "additionalProperties" has to admit the map's value schema (WriteObjectOrRecord
  // attaches it; it cannot go into the "properties" container, which only holds
  // named members)
  LAdditional := AMemberSchema.GetValue('additionalProperties');
  if Assigned(LAdditional) then
  begin
    if Assigned(AAdditionalProperties) then
    begin
      // A second unwrapped map: both value schemas hold, so they are folded
      // into one {"allOf": [<first>, <second>]}
      LCombined := TJSONObject.Create;
      LCombinedAllOf := TJSONArray.Create;
      LCombinedAllOf.AddElement(AAdditionalProperties);
      LCombinedAllOf.AddElement(LAdditional.Clone as TJSONValue);
      LCombined.AddPair('allOf', LCombinedAllOf);
      AAdditionalProperties := LCombined;
    end
    else
      AAdditionalProperties := LAdditional.Clone as TJSONValue;
    Result := True;
  end;

  // An unconstrained object (an interface member: "{type: object}" with no
  // properties known at all): the serializer still flattens whatever object the
  // implementing instance produced, so the member must not become a nested
  // property - there is simply nothing known to contribute
  if not Result then
  begin
    LType := AMemberSchema.GetValue('type');
    if Assigned(LType) and (LType is TJSONString) and (LType.Value = 'object') then
      Result := True;
  end;
end;

function TNeonSchemaGenerator.WriteMembers(AType: TRttiType; AResult: TJSONObject;
  AAllOf: TJSONArray; var AAdditionalProperties: TJSONValue): TJSONArray;
var
        ...
        try
          ...
          begin
            ...
            if LNeonMember.NeonUnwrapped then
              LUnwrapped := UnwrapMember(LJSONObj, AResult, AAllOf, AAdditionalProperties, Result);
...
var
  ...
  LAllOf: TJSONArray;
  LAdditionalProperties: TJSONValue;
  LSchema: TJSONObject;
  LDefName: string;
begin
  ...
  try
    ...
    LAllOf := TJSONArray.Create;
    LAdditionalProperties := nil;

    LRequired := WriteMembers(AType, LProperties, LAllOf, LAdditionalProperties);
    ...
    // [NeonUnwrapped] members that cannot be copied property-by-property
    // contribute here: a hoisted recursive type adds an "allOf" branch the
    // parent must also satisfy, an unwrapped map widens the parent's
    // "additionalProperties" to admit its values
    if LAllOf.Count > 0 then
      LSchema.AddPair('allOf', LAllOf)
    else
      LAllOf.Free;
